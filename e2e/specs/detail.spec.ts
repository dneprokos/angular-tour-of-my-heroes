...
test.describe.parallel('Tour of Heroes - Detail page e2e tests', async () => {
    test("Hero details id should equals to url id", async ({ page }) => {
        ...
        //Act
        const currentUrl = await detailsPage.page.url();
        const urlArray: string[] = currentUrl.split('/');
        const urlId = urlArray[urlArray.length - 1];

        const heroId: string | undefined = await detailsPage.getHeroId();
    ...
    async function navigateDetailsPageFromSpecificPage(initPageName: string, page: Page): Promise<[DetailsPage, string]> {
        ...
        switch (initPageName) {
            case 'Dashboard': {
                ...
                const dashboardPage = await navigateToDashboardPage(page);
                const firstHeroName = await (await dashboardPage.topHeroes.getTopHeroesNames()).filter(name => name)[0];
                detailsPage = await dashboardPage.topHeroes.clickHeroName(firstHeroName);
                break;
            }
            case 'Heroes': {
                expectedRedirectUrl = PageUrls.heroesPage;
                const heroesPage = await navigateToMyHeroesPage(page);
                const heroesList = heroesPage.heroesListFragment;
                const firstHero = await (await heroesList.getHeroesIdNamePairs()).filter(name => name)[0];
